...
// Schema para validação do nome do grupo
const UpdateGroupNameSchema = z.object({
  name: z.string().min(1, 'Nome é obrigatório').max(255, 'Nome muito longo'),
})

// PATCH /api/groups/[id]/name - Atualizar nome do grupo
export async function PATCH(
  ...
  try {
    ...
    if (updateError) {
      console.error('Erro ao atualizar nome do grupo:', updateError)
      return NextResponse.json(
    ...
    if (existingGroup.whatsapp_id) {
      try {
        console.log(`🔄 Sincronizando nome do grupo ${existingGroup.whatsapp_id} no WhatsApp para: ${name}`)
        ...
        if (instanceError || !userInstance) {
          ...
          // Continuar sem falhar - o nome foi atualizado no banco local
        } else {
          // Criar cliente Z-API
          ...
          if (metadataResult.success && metadataResult.data) {
            ...
            if (isAdmin) {
              ...
              // Atualizar nome do grupo no WhatsApp
              const zApiResult = await zApiClient.updateGroupName(existingGroup.whatsapp_id, name)
              
              if (zApiResult.success) {
                console.log('✅ Nome do grupo atualizado no WhatsApp com sucesso')
              } else {
                console.error('❌ Erro ao atualizar nome do grupo no WhatsApp:', zApiResult.error)
                // Não falhar a operação se a sincronização falhar
              }
            } else {
              console.log('⚠️ AVISO: Usuário não é administrador do grupo, não é possível atualizar o nome no WhatsApp')
            }
          } else {
    ...
    console.error('Erro na API de atualização de nome do grupo:', error)
    return NextResponse.json(
