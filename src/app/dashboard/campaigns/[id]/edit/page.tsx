...
export default function EditCampaignPage() {
  ...
  const fetchCampaignData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Preencher form data
        // Converter mensagens e mídias em variantes
        const variants: CampaignVariant[] = [];
        const messages = data.messages || [];
        const media = data.media || [];
        
        // Criar variantes baseadas nas mensagens
        messages.forEach((message: CampaignMessage, index: number) => {
          const variantMedia = media.filter((m: CampaignMedia) => m.media_order === message.message_order);
          variants.push({
            id: `variant_${message.id}`,
            variant_order: message.message_order,
            message: message,
            media: variantMedia || [],
        ...
        setFormData({
          ...
          send_order: data.send_order || 'text_first',
        ...
        // Carregar global_interval
        setGlobalInterval(data.global_interval || 0);
      }
    } catch (error) {
  ...
  const updateVariantMessage = (variantId: string, field: string, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Campanha executada! ${result.message}`);
      } else {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Carregando campanha...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Button
            ...
            variant="outline"
            className={formData.status === 'active' ? 'border-gray-400 text-gray-700 hover:bg-gray-100' : 'border-black text-black hover:bg-gray-100'}
          >
      ...
      <div className="flex items-center space-x-2">
        ...
        <Badge className={
          ...
          formData.status === 'paused' ? 'bg-gray-100 text-gray-900 border border-gray-300' :
          formData.status === 'completed' ? 'bg-gray-200 text-gray-900' :
          formData.status === 'cancelled' ? 'bg-gray-100 text-gray-600 border border-gray-200' :
          'bg-gray-50 text-gray-700 border border-gray-200'
        }>
          {formData.status === 'active' ? 'Ativa' :
           formData.status === 'paused' ? 'Pausada' :
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: any) => handleInputChange('send_order', value)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <Card className="border border-gray-200">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-gray-900">Variantes de Campanha</CardTitle>
                <CardDescription className="text-gray-600">
        ...
        <Card className="border border-gray-200">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-gray-900">Agendamentos</CardTitle>
                <CardDescription className="text-gray-600">
          ...
          <CardContent className="space-y-4">
            {formData.schedules.map((schedule, index) => (
              <div key={schedule.id} className="border border-gray-200 rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-medium text-gray-900">Agendamento {index + 1}</h4>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
        ...
        <Card className="border border-gray-200">
          <CardHeader>
            <CardTitle className="text-gray-900">Grupos de Destino</CardTitle>
            <CardDescription className="text-gray-600">
        ...
        {formData.selected_groups.length > 0 && (
          <Card className="border border-gray-200">
            <CardHeader>
              <CardTitle className="text-gray-900">Intervalos de Envio por Grupo</CardTitle>
              <CardDescription className="text-gray-600">
